...
final storageServiceProvider = Provider<HybridStorageService>(
  (_) => HybridStorageService(),
);
...
class BooksNotifier
  ...
  Future<void> loadBooks() async {
    ...
    try {
      // Step 1: Show local data instantly.
      final localBooks = await HiveService().getBooks(userId);

      for (final book in localBooks) {
        final bookId = book['id'] as String;
        final income = await HiveService().getTotalIncome(bookId);
        final expenses = await HiveService().getTotalExpenses(bookId);

        book['balance'] = income - expenses;
      }

      if (mounted) {
        state = AsyncValue.data(List.from(localBooks));
      }
      ...
      // Already have local data, so don't replace it with an error state.
  ...
  Future<void> _syncAllFromRemote() async {
    ...
    try {
      ...
      final remoteBooks = await supabase.getBooks(userId);
      ...
      final updatedBooks = remoteBooks.map((book) {
        ...
        final income = entries
            .where((entry) => entry['is_income'] == true)
            .fold<double>(
              0.0,
              (sum, entry) => sum + (entry['amount'] as num).toDouble(),
            );
